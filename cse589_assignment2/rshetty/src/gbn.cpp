...
*/	
			...
			printf("have to check  %d\n",count);
                        lastsent_index=lastsent_index-count;
                        int lastlimit=lastsent_index+count;
...
/*
        if(achecksum==packet.checksum && expectingack==packet.acknum)
        {
                printf("\nsame checksum\n ");
		previousack=expectingack;
 ...
 struct pkt binputpacket;
	...
	else if()
	{
        ...
        {
                printf("\ndifferent checksum or duplicate ack\n");
        }
...
void A_timerinterrupt() //ram's comment - changed the return type to void.
{
  ...
  printf("\n\n\t\tinside A_timerinterrupt\n\n");
	...
	starttimer(0,TIMEOUT);
	 ...
	 for(unsigned int i=0;i<holdingvector.size()&&i<WINSIZE;i++)
                {
	 ...
	 B_transport++;
        ...
        if(bchecksum==packet.checksum)
        {
                ...
                if(packet.seqnum==expecting)
                {
                        ...
                        B_application++;
                        printf("sending to b application\n");
